    ...
    for (auto& Entity : Core::SDK::Game::EntityList)
    {
        ...
        if (Cfg->Skeleton)
        {
            ...
            if (!Core::SDK::Game::IsOnScreen(Lfoot) || !Core::SDK::Game::IsOnScreen(Rfoot) || !Core::SDK::Game::IsOnScreen(Pelvis) ||
                ...
                !Core::SDK::Game::IsOnScreen(RightCalf))
                continue;
        ...
        if (Cfg->UserNames)
        {
            ...
            ImVec2 TextSize = poppins->CalcTextSizeA(poppins->FontSize, FLT_MAX, 0.0f, PlayerName.c_str());

            // Ajuste para mover o nome mais para a direita e para cima
            const float OffsetX = 15.0f; // Mover 10 pixels para a direita
            const float OffsetY = -20.0f; // Mover 20 pixels para cima
            ImVec2 TextPos = ImVec2(BoxCenter.x - (TextSize.x / 2) + OffsetX, FirstTextBoxTop + OffsetY);

            switch (Cfg->UserNamesState)
            {
            case 0: // Top
                TextPos.y = FirstTextBoxTop + OffsetY;
                if (Cfg->DistanceFromMe && Cfg->DistanceFromMeState == 0) {
                    TextPos.y -= 15; // Ajuste para evitar sobreposição com distância
                }
                if (Cfg->WeaponName && Cfg->WeaponNameState == 0) {
                    TextPos.y -= 15; // Ajuste para evitar sobreposição com arma
                }
                break;
            case 1: // Bottom
                TextPos.y = FirstTextBoxBottom + OffsetY + 15;
                if (Cfg->DistanceFromMe && Cfg->DistanceFromMeState == 1) {
                    TextPos.y += 15; // Ajuste para evitar sobreposição com distância
                }
                if (Cfg->WeaponName && Cfg->WeaponNameState == 1) {
                    TextPos.y += 15; // Ajuste para evitar sobreposição com arma
                }
                break;
            default:
        ...
        if (Cfg->DistanceFromMe)
        {
            ...
            ImVec2 TextSize = poppins->CalcTextSizeA(poppins->FontSize, FLT_MAX, 0.0f, DistStr.c_str());
            ...
            case 1: // Bottom
                ...
                if (Cfg->UserNames && Cfg->UserNamesState == 1) {
                    TextPos.y += 15;
                }
                if (Cfg->WeaponName && Cfg->WeaponNameState == 1) {
                    TextPos.y += 15;
                }
                if (Cfg->WeaponName && Cfg->WeaponNameState == 1 && Cfg->UserNames && Cfg->UserNamesState == 1) {
                    TextPos.y += 30;
                }
                break;
            default:
            ...
            DrawList->AddText(ImVec2(TextPos.x + 1, TextPos.y + 1), ImColor(0.f, 0.f, 0.f, Cfg->DistanceCol.Value.w), DistStr.c_str());
            DrawList->AddText(ImVec2(TextPos.x, TextPos.y), Cfg->DistanceCol, DistStr.c_str());
        }
        ...
        if (Cfg->WeaponName)
        {
            ...
            ImVec2 TextSize = poppins->CalcTextSizeA(poppins->FontSize, FLT_MAX, 0.0f, to_lower(weaponName.c_str()).c_str());
            ImVec2 TextPos = ImVec2(BoxCenter.x - (TextSize.x / 2), FirstTextBoxBottom);
            ...
            DrawList->AddText(ImVec2(TextPos.x + 1, TextPos.y + 1), ImColor(0.f, 0.f, 0.f, Cfg->WeaponNameCol.Value.w), WeaponNameUnder.c_str());
            DrawList->AddText(ImVec2(TextPos.x, TextPos.y), Cfg->WeaponNameCol, WeaponNameUnder.c_str());
        }
...
void Core::Features::cEsp::DrawVehicle()
{
    ...
    for (auto Entity : Core::SDK::Game::VehicleList)
    {
        ...
        if (Cfg->VehName)
        {
            ImVec2 text_size = poppins->CalcTextSizeA(poppins->FontSize, FLT_MAX, 0.0f, Entity.Name.c_str());
        ...
        if (Cfg->ShowLockUnlock)
        {
            ...
            if (IsLocked)
            {
                ...
                {
                    ImVec2 text_size = poppins->CalcTextSizeA(poppins->FontSize, FLT_MAX, 0.0f, VehicleLocked.c_str());
            ...
            else
            {
                ...
                if (FovSize < fovRadius)
                {
                    ImVec2 text_size = poppins->CalcTextSizeA(poppins->FontSize, FLT_MAX, 0.0f, VehicleUnlocked.c_str());
        ...
        if (Cfg->DistanceFromMe)
        {
            ...
            ImVec2 text_size = poppins->CalcTextSizeA(poppins->FontSize, FLT_MAX, 0.0f, vehicleDistance.c_str());
