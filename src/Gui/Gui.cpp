...
void legit_tab() {
	...
	ImGui::MenuChild(skCrypt("Accuracy"), ImVec2(320, 286));
	{
		...
		ImGui::SliderInt(xorstr("Smooth Horizontal"), &g_Config.Aimbot->SmoothHorizontal, 0, 100, xorstr("%d"), 0);
...
void teleports_tab() {
	...
	std::vector<Locations_t> Locations = {
		...
		Locations_t(skCrypt("Casino").decrypt(), D3DXVECTOR3(885.322, 16.8489, 80.65)),
		...
		// Funny / Troll spots
		Locations_t(skCrypt("Inside Prison").decrypt(), D3DXVECTOR3(1692.19, 2593.92, 45.89)),
		Locations_t(skCrypt("IAA Interior").decrypt(), D3DXVECTOR3(215.31, -684.49, 88.95)),
		Locations_t(skCrypt("Lester's House").decrypt(), D3DXVECTOR3(1273.91, -1712.51, 54.78)),
		Locations_t(skCrypt("Strip Club DJ Booth").decrypt(), D3DXVECTOR3(126.79, -1277.49, 34.38)),
		Locations_t(skCrypt("Lifeinvader Office").decrypt(), D3DXVECTOR3(-1048.29, -237.12, 44.02)),
	};
...
void Decoration()
{
	...
	draw->AddText(baloo, 38, ImVec2(pos.x + 19, pos.y + 24), rocketColorWithAlpha, "uwuhook");
	ImGui::SetCursorPos(ImVec2(34, 0));
	//ImGui::Image(g_Variables.Logo, ImVec2(80, 80));

	std::stringstream versionInfo;
	versionInfo << skCrypt("Ver: (").decrypt() << std::fixed << std::setprecision(1) << "Dev" << ")";
