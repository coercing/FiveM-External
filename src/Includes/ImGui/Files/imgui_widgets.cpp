...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
	...
	const ImVec2 text_pos(window->DC.CursorPos.x + 4.0f, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	it_anim->second.hovered_anim = ImLerp(it_anim->second.hovered_anim, hovered ? 1.35f : 0.5f, 0.05f * (1.0f - ImGui::GetIO().DeltaTime));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	if (*v)
		RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(4, 3), rocketColorWithAlpha, square_sz - 4);
...
void ImGui::Hotkey(int* k, const ImVec2& size_arg, bool* waitingforkey, const char* suffix, bool isMenu)
{
	if (*waitingforkey == false) {
		if (ImGui::Button((std::string(KeyNamesText[*k]) + " " + suffix).c_str(), size_arg))
			*waitingforkey = true;
	}
	else {
		ImGui::Button("waiting... (press esc to cancel)", size_arg);
		...
		if (!isMenu) {
			if (GetAsyncKeyState(Hotkey::hot().menuSwitchHotKey) & 0x8000) {
				*waitingforkey = false;
				return;
			}
		}
		...
		{
			if ((GetAsyncKeyState(KeyCode) & 0x8000) && (KeyCode >= 0x01 && KeyCode <= 0xA5)) {
				*k = KeyCode;
				*waitingforkey = false;
			}
		}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	if (grab_bb.Max.x > grab_bb.Min.x)
	{
		...
		window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 1), grab_bb.Max + ImVec2(2, 1), rocketColorWithAlpha, 3.0f);
	}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	if ((init_state && g.ActiveId != id) || init_changed_specs)
	{
		...
		if (!is_multiline)
		{
			...
			// Borda branca apenas durante hover
			if (hovered || IsItemActive())
			{
				...
				draw_window->DrawList->AddRect(
					frame_bb.Min,
					frame_bb.Max,
					GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)),
					style.FrameRounding,
					0,
					border_thickness
				);
			...
			draw_window->DrawList->AddRect(
				frame_bb.Min,
				frame_bb.Max,
				ImColor(accent_colour[0], accent_colour[1], accent_colour[2], content_animation * 0.1f + it_anim->second.hovered_anim),
				style.FrameRounding
			);
		}
...
bool ImGui::Rendertab(const char* icon, const char* label, bool selected)
{
	...
	ImColor sidebarIconsColorWithAlpha = ImColor(Gui::cOverlay.sidebarIconsColor.Value.x, Gui::cOverlay.sidebarIconsColor.Value.y, Gui::cOverlay.sidebarIconsColor.Value.z, Gui::cOverlay.sidebarIconsColor.Value.w * ImGui::GetStyle().Alpha);

	ImColor sidebarSelectedIconsColorWithAlpha = ImColor(Gui::cOverlay.sidebarSelectedIconColor.Value.x, Gui::cOverlay.sidebarSelectedIconColor.Value.y, Gui::cOverlay.sidebarSelectedIconColor.Value.z, Gui::cOverlay.sidebarSelectedIconColor.Value.w * ImGui::GetStyle().Alpha);
	...
	GetWindowDrawList()->AddText(font_icon, 18, ImVec2(bb.Min.x + 10, bb.Min.y + 5), sidebarIconsColorWithAlpha, icon);
	...
	if (selected)
	{
		...
		GetWindowDrawList()->AddRectFilled(ImVec2(pos.x, pos.y + 5), ImVec2(pos.x + it_fill->second * 2, pos.y + 25), rocketColorWithAlpha, 10.f, ImDrawCornerFlags_Right);

		GetWindowDrawList()->AddText(font_icon, 18, ImVec2(bb.Min.x + 10, bb.Min.y + 5), sidebarSelectedIconsColorWithAlpha, icon);
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	...
	parent_window->DrawList->AddRectFilled(pos, p + size_arg, ImColor(22, 22, 22, int(255 * ImGui::GetStyle().Alpha)), 5);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	const ImU32 col_hues[6 + 1] = { IM_COL32(255, 255, 255,style_alpha8), IM_COL32(255,255,0,style_alpha8), IM_COL32(0,255,0,style_alpha8), IM_COL32(0,255,255,style_alpha8), IM_COL32(0,0,255,style_alpha8), IM_COL32(255,0,255,style_alpha8), IM_COL32(255, 255, 255,style_alpha8) };
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	it_hovered->second = ImClamp(it_hovered->second + (2.0f * ImGui::GetIO().DeltaTime * (hovered ? 1.0f : -3.0f)), 0.0f, 1.0f);
	...
	it_active->second = ImClamp(it_active->second + (3.0f * ImGui::GetIO().DeltaTime * (selected ? 1.0f : -1.5f)), 0.0f, 1.0f);
	...
	// Render
	window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 4 * it_active->second, bb.Max.y), rocketColorWithAlpha, 10.f, ImDrawCornerFlags_Right);

	RenderTextClipped(text_min, text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);

	// Overlay highlight color for hovered or selected item
	if (hovered || selected)
	{
		ImVec2 text_pos = ImVec2(ImClamp(text_min.x, bb.Min.x, bb.Max.x), ImClamp(text_min.y, bb.Min.y, bb.Max.y));
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
	...
	while (clipper.Step())
	{
		for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
		{
			...
			// Remove padding to get tight bounds
			...
			ImVec2 text_pos = GetCursorScreenPos();

			// Render selectable
			bool item_clicked = Selectable(item_text, item_selected, 0, text_size);

			// Overlay strong white text if hovered or selected
			if (IsItemHovered() || item_selected)
			{
				GetWindowDrawList()->AddText(text_pos, IM_COL32_WHITE, item_text);
			}
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, const ImGuiTabItem* src_tab, ImVec2 mouse_pos)
{
	...
	for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir)
	{
		...
		//GetForegroundDrawList()->AddRect(ImVec2(x1, tab_bar->BarRect.Min.y), ImVec2(x2, tab_bar->BarRect.Max.y), IM_COL32(255, 255, 255, 255));
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
	...
	//window->DrawList->AddRect(ImVec2(tab_bar->BarRect.Max.x - scrolling_buttons_width, tab_bar->BarRect.Min.y), ImVec2(tab_bar->BarRect.Max.x, tab_bar->BarRect.Max.y), IM_COL32(255, 255, 255,255));
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
	...
	if (out_text_clipped)
	{
		*out_text_clipped = (text_ellipsis_clip_bb.Min.x + label_size.x) > text_pixel_clip_bb.Max.x;
		//draw_list->AddCircle(text_ellipsis_clip_bb.Min, 3.0f, *out_text_clipped ? IM_COL32(255, 255, 255, 255) : IM_COL32(0, 255, 0, 255));
	}
