...
namespace NotifyManager
{
    ...
    inline void Render( )
	{
        ...
        for ( auto i = 0; i < NotifyList.size( ); i++ )
        {
            ...
            auto TitleTxtSize = Utils::CalcTextSize( poppins, poppins->FontSize, Notify.GetTitle( ).c_str( ) );
            auto DescTxtSize = Utils::CalcTextSize(poppins, poppins->FontSize, Notify.GetDescription( ).c_str( ) );
            ...
            DrawList->AddText(poppins, poppins->FontSize, TitlePos, ImGui::GetColorU32( ImVec4( ImColor( g_Col.PrimaryText ) ) ), Notify.GetTitle( ).c_str( ) );
            DrawList->AddText(poppins, poppins->FontSize, TitlePos + ImVec2( 0, TitleTxtSize.y + 2.f ), ImGui::GetColorU32( ImVec4( ImColor( g_Col.SecundaryText ) ) ), Notify.GetDescription( ).c_str( ) );
    ...
    inline void Send( std::string Description, time_t ExpireTime )
    {
        ...
        Notify.SetTitle( xorstr( "uwuhook.club" ) );
